...
### Done (2026-04-11)
- [x] Reactive collection via `checkpoints.include_explicit` — toggling `include_in_collection` scans or clears sightings immediately, active tips auto-include via `reconcileTipsInclusion()` hooked into `syncSaves` + startup, identity GC cleans orphaned rows. Settings → Collection has a manual "Reconcile collection" escape hatch (2026-04-13)
...
- [x] ~~**Investigate parallel `pokemon`-table save-import pipeline**~~ — Retired 2026-04-13. The `pokemon` table and its pipeline (`parseAndImportAll`, `runCompletionScan`, `POST /api/pokemon/sync*`, `POST /api/guide/scan-completion`) were all confirmed dead (the Guide reads from `guide_progress` + `specimen_*` tables, not `pokemon`). Table dropped via migration, dead code deleted, and a `specimen_progress.pokemon_id REFERENCES pokemon(id)` FK was stripped via a create/copy/drop/rename rebuild. See `docs/superpowers/plans/2026-04-13-collection-reconciliation.md`.
...
- [x] ~~**Pokedex sync button 404s on `/saves/collection`**~~ — Retired 2026-04-13. The orphaned `SyncCollectionDialog` was fully deleted along with `api.pokemon.sync` / `api.pokemon.syncPreview`. The collection now stays in sync reactively — import a save, toggle a checkpoint, or advance a tip and the Pokedex updates on its own. A manual "Reconcile collection" escape-hatch button lives in Settings → Collection for edge cases. See `docs/superpowers/plans/2026-04-13-collection-reconciliation.md`.
